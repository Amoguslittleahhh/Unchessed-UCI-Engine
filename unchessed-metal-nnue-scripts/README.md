...
Run the pilot first. **Note**: `--game-field none` below gives every
position its own synthetic single-position "game", so `split_manifest.py`'s
game-disjoint splitting is a no-op on pilot data -- fine for a pilot
smoke-test, but for a real run pass `--game-field <column>` pointing at an
actual game identifier, or positions from the same real game can leak
