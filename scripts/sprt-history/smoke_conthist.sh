...
"$CUTECHESS" \
  -engine cmd="$ENGINE" name=A option.Threads=1 option.Adaptive=false option.OwnBook=false option.Hash=64 option.ContHistPct=100 \
  -engine cmd="$ENGINE" name=B option.Threads=1 option.Adaptive=false option.OwnBook=false option.Hash=64 option.ContHistPct=100 \
