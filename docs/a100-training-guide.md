...
only. A later data-format revision should add promotion identity, game/player
hashes, time class, WDL, and recent position history.
...
Run these before allocating a long job. The first compiled training step can be
...
Shell expansion is intentional; avoid spaces in shard paths. The jobs run
