# Engineering request: make the Unarchitectured Metal runtime forward pass fast enough to actually use

## Scope note

Unarchitectured Metal is the canonical, current architecture — the target to
...
Unarchitectured Metal, not as a parallel track exploring an earlier one.
...
**Unarchitectured Metal runtime speed (the original ask)**: round 18's
...
2. **A product decision on `PersonaSmooth`/`EngineDetectV2` defaults.**
   ...
   owner's call.
3. **A retrain decision** for round 9's Unarchitectured Metal findings (GAB
...
- **Round 0**: root-hint wired directly into search, SPRT'd, failed
  ...
  unwired forward-pass module (`unarchitectured_metal_runtime.rs`) survived.
...
- **Round 18**: rejected, not merged. 9 real compile errors in an
  `unarchitectured_metal_runtime.rs` GEMM-tiling refactor. **Round 19** fixed all of
...
Any change to `unarchitectured_metal_runtime.rs` must keep passing
`start_position_matches_python_reference`,
...
`tools/reference_forward_unarchitectured_metal.py`. Don't trade correctness
...
- `unchessed-core/src/unarchitectured_metal_runtime.rs` — the runtime module, forward
  pass, and all correctness tests.
- `tools/reference_forward_unarchitectured_metal.py` — the independent
  Python reference. Keep in sync with any intentional behavior change.
- `unchessed-core/src/unarchitectured_metal.rs` — the binary package format.
- `tools/train_unarchitectured_metal_student_a100.py` — the original
