...
| **NNUE label-noise comparison** [4] | A real paired live-generation measurement found **17–22 cp MAE** and **Pearson 0.93–0.98** across four PGN sources and 10×/20× depth multipliers. This contradicts the former modelled **50–56 cp** floor at the tested scale; the old-label explanation is reopened, not settled. | Reviewer-run datagen side-channel compared the 5,000-node primary score with a second search on the same accepted legal position; four samples totalled **2,900 paired records**. The historical report and commits `6c8513c`/`6667f6f` were inspected; no expensive rerun was performed here. | No existing-108M-corpus relabel, labels synthesized, retraining, match, or SPRT. The sample is narrow and does not prove deeper-score correctness or Elo benefit. | **Reopen, bounded.** Retain the full-state/provenance blocker for old-shard relabeling. Optionally pursue a preregistered extension only for a specific unresolved question; prioritize effective-coverage/capacity analysis. No retrain/default change without normal real paired-game gates. |
