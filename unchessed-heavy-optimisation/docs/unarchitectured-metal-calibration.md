# Unarchitectured Metal provenance-disjoint calibration

This is round 6 step 1: calibration of the exported Unarchitectured Metal student
...
| Component | Value |
|---|---|
| Model | `artifacts/unarchitectured-metal-final.unmetal` (real exported checkpoint) |
| Forward pass | `tools/reference_forward_unarchitectured_metal.py` (the repo's validated PyTorch reference, cross-checked against the Rust runtime at 5e-3) |
...
Both corpora are committed (`artifacts/unarchitectured-metal-calibration-corpus.jsonl`,
`...-replication.jsonl`) along with the full reports
(`artifacts/unarchitectured-metal-calibration-report.json`, `...-replication.json`).
...
4. **Paired-game SPRT** (step 5) via the existing
   `scripts/sprt-history/sprt_unarchitectured_metal_hint.sh`, testing the **full
