...
The original prose interpreted the sign backwards. `cutechess-cli` reports the score and Elo difference relative to the first-named engine, which was `main`. The reported result therefore indicates approximately $-244.4 plus.minus 48.0$ Elo for `main` relative to the heavy branch, with 0.0% likelihood of `main` superiority. The accompanying termination summary reported 146 losses and 18 wins for `main`. Neither side produced an illegal move, disconnect, crash, or time forfeit. Twelve games per side were aborted in flight after the boundary was reached; these were test termination events rather than engine failures.

A second supplied 200-game low-concurrency control reported `Score of Main vs HeavyOpt: 21 - 140 - 39 [0.203]` and an Elo difference of $-238.1 plus.minus 50.7$ relative to `main`. Together, the supplied matches indicate that heavy optimization outperformed `main` by roughly 240 Elo under Adaptive=true. This establishes aggregate branch strength, but it does not prove that every individual heavy-branch change is beneficial because several modifications remain coupled.
