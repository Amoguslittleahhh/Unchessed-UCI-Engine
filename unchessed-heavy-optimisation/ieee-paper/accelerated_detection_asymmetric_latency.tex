...
In the Stockfish 19 clock-controlled run, the resilient reason appeared in both enabled games, with first-Full plies 26 and 24; the standard arm confirmed once at ply 30 and once beyond the 32-ply horizon. All four games produced 15 observations and zero low-time skips. In the first Maia-3 run, both arms exposed early legacy-clock confirmations; this was not accepted as evidence for the resilient lane. After tightening accelerated clock corroboration, the Maia-3 resilient arm produced one no-confirmation game and one stable-fusion confirmation at ply 26. No game produced an illegal move or a process failure. This is an operational and targeted-channel result, not proof of universal superiority.
...
The experiment does not support the stronger claim that AcceleratedDetection is uniformly better. The earlier 80 ms repository run was 0.6 plies worse on average, while the new Stockfish-19 run has only two games per arm. The Maia-3 probe is four games total and shows that the legacy clock path can still false-confirm against a human-like model; the accelerated clock corroboration fix reduces that risk but is not a false-positive estimate. The bounded four-game strength SPRT reached the continuation region with LLR 0.000 and unfinished capped games, so it cannot establish strength neutrality. The low-time trap also shows that a latency result is uninterpretable when observation coverage is zero. Results remain conditional on fixed openings, one thread, short horizons, and a detector-facing opponent string that avoids the known-engine shortcut. False-positive rate against humans and generalization across engine families remain unmeasured.
...
\section{Conclusion}
This work provides a real, telemetry-backed asymmetric evaluation and a targeted correction to the AcceleratedDetection fusion model in the Unarchitectured Metal series. The implementation is lightweight, interpretable, dependency-free, and now has both a volatility-sensitive stable lane and a volatility-tolerant resilient lane. The independent replication and corrected clock-controlled run support the methodological requirement that observation coverage be reported explicitly. The evidence shows occasional earlier confirmation and robust operation, but remains mixed rather than flawless. A larger Maia-3 evaluation, a completed paired strength SPRT, and an explicit policy decision for the non-accelerated legacy-clock path are required before any universal latency or promotion claim.
