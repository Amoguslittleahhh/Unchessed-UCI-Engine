...
//! `unarchitectured_metal_runtime.rs`; this module remains responsible for package parsing
//! and validation only.

use std::path::Path;

/// Canonical unarchitectured-metal container identifier.
pub const MAGIC: &[u8; 8] = b"UNMETAL1";
/// Legacy v1 identifier accepted for backward-compatible loading.
pub const LEGACY_MAGIC: &[u8; 8] = b"UNARCHV1";
...
impl<'a> TensorPackage<'a> {
    pub fn parse(bytes: &'a [u8]) -> Result<Self, String> {
        ...
        if &bytes[..8] != MAGIC && &bytes[..8] != LEGACY_MAGIC {
            return Err("bad UNMETAL1/UNARCHV1 magic".into());
        }
...
mod tests {
    ...
    fn fixture() -> Vec<u8> {
        let metadata = br#"{"architecture":"Unarchitectured Metal"}"#;
