//! Pure-Rust inference forward pass for the canonical Unarchitectured Metal
//! compact student (training class `UnarchitecturedV1Student`), exported to a
//! UNARCHV1 package (see `unarchitectured_metal.rs` for the container format).
...
//!
//! Ported line-for-line from `tools/train_unarchitectured_metal_student_a100.py`'s
...
fn dequantize(section: &crate::unarchitectured_metal::TensorSection) -> Vec<f32> {
...
impl ChessformerWeights {
    pub fn from_package(pkg: &TensorPackage) -> Result<Self, String> {
        ...
        for section in &pkg.sections {
            ...
            if section.is_quantized()
                && (section.dtype != crate::unarchitectured_metal::DTYPE_I8 || section.zero_point != 0)
            {
                return Err(format!(
            ...
            if !section.is_quantized() && section.dtype != crate::unarchitectured_metal::DTYPE_F32 {
                return Err(format!(
...
/// `unarchitectured_metal_data.encode_action` (source | target<<6 | promotion<<12, with
...
mod tests {
    ...
    /// Cross-checked against tools/reference_forward_unarchitectured_metal.py run on the
    /// same real exported checkpoint (artifacts/unarchitectured-metal-final.unmetal):
    ///   legal_count 20
    ...
    /// Cross-checked against `tools/reference_forward_unarchitectured_metal.py
    /// artifacts/unarchitectured-metal-final.unmetal --all-exits` on the same
