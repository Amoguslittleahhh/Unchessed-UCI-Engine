#!/usr/bin/env python3
"""Train the canonical Unarchitectured Metal elastic runtime student on A100.
...
def argument_parser():
    ...
    parser.add_argument("--config", default="config/unarchitectured_metal_student.json")
    parser.add_argument("--train", nargs="+", default=[])
    parser.add_argument("--calibration", nargs="+", default=[])
    parser.add_argument("--validation", nargs="+", default=[])
    parser.add_argument("--output", type=Path, default=Path("unarchitectured-metal-student.pt"))
...
from unarchitectured_metal_data import (
    ACTION_SENTINEL,
    HEADER_BYTES,
    LEGAL_FLAG_REGRETS,
    MAGIC,
...
class UnarchitecturedV1RecordShards:
    """Header-validated random access over frozen Unarchitectured Metal records."""
    ...
    def __init__(self, paths):
        ...
        if not self.shards:
            raise ValueError("no non-empty Unarchitectured Metal shards")
...
def prepare_batch(records, device, augment=False):
    ...
    if not bool(target_matches.any(1).all()):
        raise RuntimeError("Unarchitectured Metal target action is absent from legal action set")
    ...
    if labelled_rows.any() and not bool(teacher_matches[labelled_rows].any(1).all()):
        raise RuntimeError("Unarchitectured Metal teacher-best action is absent from labelled legal set")
...
def train(args):
    ...
    if abs(sum(config["loss_weights"].values()) - 1.0) > 1e-12:
        raise ValueError("Unarchitectured Metal student loss weights must sum to one")
...
if __name__ == "__main__":
    ...
    if arguments.command == "selfcheck":
        ...
        if not arguments.train or not arguments.calibration or not arguments.validation:
            raise SystemExit("train requires separate --train, --calibration, and --validation Unarchitectured Metal shards")
        train(arguments)
