#!/usr/bin/env python3
"""Unarchitectured Metal legal-set data ABI and leakage validator.
...
try:
    from unarchitectured_metal_base_data import (
        UnarchitecturedV1BaseRecord,
        FLAG_TEACHER,
        FormatError,
        RECORD as V3_RECORD,
    )
except ModuleNotFoundError:  # imported as tools.unarchitectured_metal_data in unit tests
    from tools.unarchitectured_metal_base_data import (
        UnarchitecturedV1BaseRecord,
        FLAG_TEACHER,
        FormatError,
        RECORD as V3_RECORD,
...
def mirror_action(action: int) -> int:
    if not 0 <= action < ACTION_VOCABULARY:
        raise FormatError("action outside Unarchitectured Metal vocabulary")
    source = (action & 63) ^ 7
...
class UnarchitecturedV1Record:
    ...
    def validate(self) -> None:
        ...
        if self.reserved != bytes(48):
            raise FormatError("reserved Unarchitectured Metal tail bytes must be zero")
...
def parse_header(payload: bytes) -> int:
    ...
    if (version, header_bytes, record_bytes) != (VERSION, HEADER_BYTES, RECORD_BYTES):
        raise FormatError("unsupported Unarchitectured Metal wire version/header/record width")
