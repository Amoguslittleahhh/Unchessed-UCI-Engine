...
class EncoderTests(unittest.TestCase):
    """The encoder must agree with `position_to_input` in unarchitectured_metal_runtime.rs."""

    @unittest.skipUnless(HAVE_CHESS, "python-chess not installed")
    def test_self_check_passes(self):
        result = subprocess.run(
            [sys.executable, str(TOOLS / "unarchitectured_metal_position_encoding.py"), "--self-check"],
            capture_output=True,
            text=True,
        )
    ...
    def test_start_position_matches_frozen_fixture(self):
        module = load_module("_encoding", TOOLS / "unarchitectured_metal_position_encoding.py")
        board = chess.Board()
...
class CorpusBuilderTests(unittest.TestCase):
    ...
    def test_fixture_fens_are_excluded(self):
        module = load_module(
            "_corpus", TOOLS / "build_unarchitectured_metal_calibration_corpus.py"
        )
    ...
    def test_help_runs_standalone(self):
        result = subprocess.run(
            [
                sys.executable,
                str(TOOLS / "build_unarchitectured_metal_calibration_corpus.py"),
                "--help",
            ],
            capture_output=True,
            text=True,
...
class StatisticsTests(unittest.TestCase):
    ...
    def load_calibrator(self):
        path = TOOLS / "calibrate_unarchitectured_metal_policy.py"
        try:
    ...
    def test_default_policy_kind_matches_encoder_constant(self):
        ...
        module = self.load_calibrator()
        encoding = load_module("_encoding", TOOLS / "unarchitectured_metal_position_encoding.py")
