...
MODULE_PATH = TOOLS / "unarchitectured_metal_safety.py"
SPEC = importlib.util.spec_from_file_location("unarchitectured_metal_safety", MODULE_PATH)
MODULE = importlib.util.module_from_spec(SPEC)
sys.modules[SPEC.name] = MODULE
SPEC.loader.exec_module(MODULE)
ROOT = TOOLS.parent
POLICY = json.loads((ROOT / "config/unarchitectured_metal_safety.json").read_text())
...
class UnarchitecturedV1SafetyTests(unittest.TestCase):
    ...
    def test_atomic_heartbeat_is_self_describing(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(payload["architecture"], "Unarchitectured Metal")
