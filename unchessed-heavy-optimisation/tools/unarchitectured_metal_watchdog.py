#!/usr/bin/env python3
"""Supervise an Unarchitectured Metal job without human monitoring."""
...
from unarchitectured_metal_safety import TrainingSafetyController, atomic_json
    ...
    try:
        ...
        if not heartbeat_seen and heartbeat.exists():
            try:
                last_payload = json.loads(heartbeat.read_text(encoding="utf-8"))
                heartbeat_seen = (
                    last_payload.get("architecture") == "Unarchitectured Metal"
...
def main():
    ...
    parser.add_argument(
        "--policy", default="config/unarchitectured_metal_safety.json"
    )
