#!/usr/bin/env python3
"""Train canonical Unarchitectured Metal's 58M offline oracle and distill the runtime student.
...
canonical Unarchitectured Metal runtime student. `--auto-batch` probes real forward,
...
def argument_parser():
    ...
    parser.add_argument("--config", default="config/unarchitectured_metal_training.json")
    parser.add_argument("--student-config")
    parser.add_argument("--train", nargs="+", default=[])
    parser.add_argument("--validation", nargs="+", default=[])
    parser.add_argument("--oracle", type=Path)
    parser.add_argument("--student", type=Path)
    parser.add_argument("--output", type=Path, default=Path("unarchitectured-metal-oracle.pt"))
...
from unarchitectured_metal_data import LEGAL_FLAG_REGRETS, POLICY_GUIDE, POLICY_HUMAN
from unarchitectured_metal_safety import TrainingSafetyController, atomic_json, write_heartbeat
from train_unarchitectured_metal_student_a100 import (
    ...
    def __init__(self, seed, deterministic=False):
        ...
        if self.world_size > 1:
            if not torch.cuda.is_available():
                raise RuntimeError("multi-GPU Unarchitectured Metal training requires CUDA/NCCL")
...
def oracle_loss(output, batch, config, global_step):
    ...
    # (unarchitectured_metal_uci_teacher_worker.py caps it at 32766, not at a normal-eval range) --
    ...
    # init before regret_log_scale has learned to compensate. Clamp only the
    # squared-error target here; guide_policy's softmax already saturates
    # safely on the unclamped value, and evaluation code reads the honest
    # unclamped batch["regret_target"] directly.
    regret_target_ceiling = config.get("regret_target_clip", 10.0)
    clamped_target_regret = target_regret.clamp(max=regret_target_ceiling)
    difference = clamped_target_regret - output["regret_mean"]
    regret_nll = 0.5 * (
...
def train_oracle(args):
    ...
    if config.get("epoch_mode") != "without_replacement":
        raise ValueError("Unarchitectured Metal requires without-replacement epoch semantics")
    ...
    if train_data.total < minimum_records:
        raise ValueError(
            f"training set has {train_data.total:,} records; Unarchitectured Metal requires at least "
...
def distill_student(args):
    ...
    if distill_config.get("epoch_mode") != "without_replacement":
        raise ValueError("Unarchitectured Metal distillation requires without-replacement epochs")
    require_distinct_shards(args.train, args.validation)
...
def calibrate_student_checkpoint(args):
    ...
    from train_unarchitectured_metal_student_a100 import fit_regret_calibration
...
def evaluate_checkpoint(args, student=False):
    ...
    if student:
        ...
        model = UnarchitecturedV1Student(config).to(device)
        model.load_state_dict(checkpoint_data["model"])
        from train_unarchitectured_metal_student_a100 import evaluate
...
def synthetic_records(count, seed=11):
    from train_unarchitectured_metal_student_a100 import synthetic_records as student_synthetic
...
if __name__ == "__main__":
    ...
    if arguments.command == "selfcheck":
        ...
        if not arguments.train or not arguments.validation:
            raise SystemExit("train-oracle requires --train and --validation Unarchitectured Metal guide/human shards")
