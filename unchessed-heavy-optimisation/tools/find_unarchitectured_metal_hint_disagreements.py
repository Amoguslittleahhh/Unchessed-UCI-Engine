#!/usr/bin/env python3
"""Find real positions where the Unarchitectured Metal policy disagrees with the
...
1. Encode it exactly as the runtime does (`unarchitectured_metal_position_encoding`,
   the same module the Rust port is validated against).
2. Run the real exported checkpoint through the reference forward pass
   (`reference_forward_unarchitectured_metal.forward`).
...
Usage
-----
    python3 tools/find_unarchitectured_metal_hint_disagreements.py \\
        artifacts/unarchitectured-metal-final.unmetal
    python3 tools/find_unarchitectured_metal_hint_disagreements.py PACKAGE --json
...
def analyse(package: Path, rating: int, time_class: int, policy_kind: int):
    from reference_forward_unarchitectured_metal import forward, read_package
...
def main() -> int:
    ...
    ap.add_argument("package", type=Path, help="path to the .unmetal checkpoint")
