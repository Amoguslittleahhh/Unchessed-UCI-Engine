#!/usr/bin/env python3
"""Export a calibrated Unarchitectured v1 student checkpoint to UNARCHV1."""
...
from unarchitectured_v1_package import (
    DTYPE_F32,
    DTYPE_I8,
    FLAG_QUANTIZED,
    Section,
...
def export_checkpoint(checkpoint_path, architecture_path, allow_legacy=False):
    ...
    if not checkpoint_format.startswith("UNARCHV1_") and not allow_legacy:
        raise ValueError(
            f"checkpoint format {checkpoint_format!r} is not canonical Unarchitectured v1"
        )
...
def main():
    ...
    parser.add_argument(
        "--architecture", default="config/unarchitectured_v1.json", type=Path
    )
