...
RUBI_LIMIT_NPS = 10000  # calibrated 2026-08-09 to ~50% vs Adapter(UCI_Elo=2400): 8000->55.0%, 16000->40.0%

GAME_NUM = int(sys.argv[1])
ADAPTER_IS_WHITE = sys.argv[2] == "white"
OUTDIR = f"/home/amogusontheterminal/unchessed-ai/results/tenmin_matched_level_series/game{GAME_NUM}"
...
# Matched-strength series: adapter fixed at UCI_Elo=2400 (UCI_LimitStrength
# on, Adaptive off so the target is a stable fixed level, not a dynamic
# opponent-modeled one), RubiChess throttled via LimitNps to the value
# freshly calibrated (this session) to land near 50% against that level --
# NOT full-strength RubiChess, unlike every prior exhibition series.
setup(adapter, {"Threads": 1, "Adaptive": "false", "UCI_LimitStrength": "true",
                "UCI_Elo": 2400, "OwnBook": "false", "Hash": 128})
setup(rubi, {"Threads": 1, "Hash": 128, "LimitNps": RUBI_LIMIT_NPS})
...
while ply < MAX_PLIES:
    if board.is_game_over(claim_draw=True):
        ...
        break
    is_white = (ply % 2 == 0)
    engine = adapter if (is_white == ADAPTER_IS_WHITE) else rubi
...
game = chess.pgn.Game()
game.headers["White"] = "UnchessedAdapter2400" if ADAPTER_IS_WHITE else "RubiChessNps10000"
game.headers["Black"] = "RubiChessNps10000" if ADAPTER_IS_WHITE else "UnchessedAdapter2400"
...
adapter.quit()
rubi.quit()
print("DONE, game", GAME_NUM, "adapter_white=", ADAPTER_IS_WHITE, "result:", result)
